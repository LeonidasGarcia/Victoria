...
class SimulateScreen(Frame):
    def __init__(self, master, victoria_units: List[Victoria], program_count: int,
                 reference_trace: List[ReferenceTraceModel], **kwargs):
        ...
        spacer = tk.Frame(controls_frame, bg=victoria_background)
        spacer.grid(row=0, column=1, sticky="ew")

        buttons_frame = tk.Frame(controls_frame, bg=victoria_background)
        ...
        self.play_button = tk.Button(buttons_frame, bg=victoria_background, fg="white", text="Play",
                                     command=self.resume_cells)
        self.play_button.grid(row=0, column=0, sticky="ew")

        self.pause_button = tk.Button(buttons_frame, bg=victoria_background, fg="white", text="Pause",
                                      command=self.stop_cells)
        self.pause_button.grid(row=0, column=1, sticky="ew")

        self.reset_button = tk.Button(buttons_frame, bg=victoria_background, fg="white", text="Reset",
                                      command=self.reset_cells)
        self.reset_button.grid(row=0, column=2, sticky="ew")

        self.cancel_button = tk.Button(buttons_frame, bg=victoria_background, fg="white", text="Cancel",
                                       command=lambda: self.master.load_form_screen())
    ...
    def cells_frame_setup(self):
        ...
        for i, victoria in enumerate(self.victoria_units):
            victoria_cell = VictoriaCell(cells_frame, victoria=victoria, program_count=self.program_count,
                                         program_colors=self.program_colors, reference_trace=self.reference_trace)
